...
namespace SoftifyGEO.API.SQL_Query
{
    public class ProductDemoQuery
    {
        ...
        public string ProductAdd(Product model)
        {
            ...
            var Query = "SELECT  cast(Isnull(MAX(ProductId),0) + 1 AS float)  AS ProductId FROM tbl_ProductDemo";
            var NewId = CoreSQL.CoreSQL_GetDoubleData(Query);

                var sqlQuery = "Insert Into tbl_ProductDemo ( SupplierId, ProductId, ProductName, OPQty, SellPrice, UnitId, CostPrice, Currency, Total, LUserId, ComId, Weight, ROL , BrandId)" +
        ...
        public string ProductUpdate(Product model)
        {
            ...
            var sqlQuery = "Update tbl_ProductDemo set ProductName = '" + model.ProductName + "', SupplierId='" +
        ...
        public string ProductDelete(int Id)
        {
            ...
            var sqlQuery = "Delete tbl_ProductDemo where  Productid = '" + Id + "'";
